function[Pitch_final, num_frames, r] = yin(Data, Fs, p)
%function r=yin(x,p)
...
% Alain de CheveignÅE CNRS/Ircam, 2002.
...
minf0 = 50;		% Hz - minimum frequency
maxf0 = 500;		% Hz - maximum frequency
wsize = Fs/minf0; 	% s - integration window size
lpf = Fs/4;		% Hz - lowpass prefiltering cutoff
...
hop = 200;		% samples - interval between estimates (10ms) 
...
nsamples = length(Data);% The number of sample
...
if ~isfield(p, 'range') | isempty(p.range); p.range=[1 nsamples]; end
...
f0 = (p.sr ./ prd); 	% convert to Hz
% Hongbing: Only use the best values, reference the follwing plot process
%f0(find(ap0>p.plotthreshold*2)) = 0;
...
% Hongbing: return the pitch values we need
Pitch_final = f0;
num_frames  = length(f0);
